...
namespace Bilayer {
    ...
    */
    ...
    public:
        typedef typename Tpetra::Map<types::loc_t, types::glob_t, Node>       Map;
        typedef typename Tpetra::CrsGraph<types::loc_t, types::glob_t, Node>  SparsityPattern;
        ...
        Teuchos::RCP<const SparsityPattern>    make_sparsity_pattern_transpose_interpolant(  const types::block_t range_block, 
        ...
        /**
         * Domain block-wise information about the number of lattice points, unit cell nodes, orbitals. 
         */
        types::loc_t    n_lattice_points(       const types::block_t block) const;
        types::loc_t    n_locally_owned_points( const types::block_t block) const;
        types::loc_t    n_dofs_each_point(      const types::block_t block) const;
        types::loc_t    n_cell_nodes()                                      const;
        size_t          n_orbitals(             const types::block_t block) const;

        /**
         * Range block-wise information about relevant columns (orbitals). 
         */
        Teuchos::Range1D column_range(  const types::block_t block) const;


        bool is_locally_owned_point(            const types::block_t block, 
                                                const types::loc_t lattice_index) const;
        types::subdomain_id point_owner(        const types::block_t block, 
                                                const types::loc_t lattice_index) const;
  
        /**
         * Accessors to traverse the lattice points level of discretization.
         */
        const PointData &    locally_owned_point(    const types::block_t block, 
                                                     const types::loc_t local_index) const;

        /**
         * Some basic information about the DoF repartition, available after their distribution.
         */
        types::glob_t               n_dofs() const;
        types::glob_t               n_locally_owned_dofs()  const;
        Teuchos::RCP<const Map>     locally_owned_dofs() const;
        Teuchos::RCP<const Map>     transpose_domain_map( const types::block_t block) const;
    ...
    private:
         ...
         * organized as one MultiVector according to the following structure.
         *
         *                     < range orbitals of layer 1 >        < range orbitals of layer 2 >
         *  block (0,0) <= [ Lattice 1 (intralayer terms 1->1) ][ Lattice 1 (interlayer terms 1->2) ] => block (1,0)
         *  block (0,1) <= [ Lattice 2 (interlayer terms 2->1) ][ Lattice 2 (intralayer terms 2->2) ] => block (1,1)
         *
         */
         ...
         *
         *          layer(0).n_orbitals   * unit_cell(1).n_nodes      [block *,0],
         *          layer(1).n_orbitals   * unit_cell(0).n_nodes      [block *,1];
         *
         */

        std::array<std::vector<types::glob_t> ,2>    lattice_point_dof_partition_;
        std::array<std::vector<PointData> ,2>        lattice_points_;

        /**
         * Information on the overall degrees of freedom for each range block, grouping both row blocks 
         */
        Teuchos::RCP<const Map>    owned_dofs_;

        /**
         * Information about the column blocks individual data distribution, for the transpose interpolant 
         */
        std::array<Teuchos::RCP<const Map>, 2>                  transpose_domain_maps_;
